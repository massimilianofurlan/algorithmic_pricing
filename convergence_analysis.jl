...
function convergence_analysis(last_epoch::Array{Int32,1}, last_memory::Array{Int32,3}, greedy_policy::Array{Int32,3})
	...
	cycle_states = Array{Int32,3}(undef, max_cycle_length, n_agents, n_sessions)
...
function compute_cycles(memory, greedy_policy, cycle_states, cycle_prices, cycle_profits, cycle_length, max_cycle_length)
	...
	if cycle_start != 1                                                         		# some sessions do not end at convergence
		start_ = cycle_start
		end_ = min(start_ + cycle_start - 1, n_states+1)                   				# overwerite with zeros non cycling states/prices/profits
...
function isvisited(cycle_states, state)
	for t in 1:size(cycle_states,1)
...
function get_price(state, greedy_policy, p = Array{Int8,1}(undef,n_agents))
	# get price given current greedy_policy
	for i in 1:n_agents
		p[i] = greedy_policy[state[i],i]
	end
	return p
end
