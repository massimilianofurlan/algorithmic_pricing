...
function update_q(Q::Array{Float32,3}, V::Array{Float32,2}, s::Int32, p::Array{Int8,1}, profits::Tuple, next_state::Int32, greedy_policy::Array{Int8,2}, alloc_max_idx::Array{Int8,1})
	...
	for i in 1:n_agents
		Q[s,p[i],i] = (1 - alpha[i]) * Q[s,p[i],i] + alpha[i] * (profits[i] + delta[i] * V[next_state,i])  # update Q
		if Q[s,p[i],i] <= V[s,i] && greedy_policy[s,i] == p[i]
			greedy_policy[s,i] = rand(argmax_(view(Q,s,:,i), alloc_max_idx))						# current p[i] may not be best price anymore (arbitrarily break ties)
		elseif Q[s,p[i],i] >= V[s,i] && greedy_policy[s,i] != p[i]
			greedy_policy[s,i] = p[i]																# current p[i] is best price in state s
		end
...
function get_price(state::Int32, greedy_policy::Array{Int8,2}, epsilon::Array{Float64}; p = Array{Int8,1}(undef,n_agents))
	# get price given current greedy_policy under e-greedy policy
	for i in 1:n_agents
		p[i] = ifelse(rand() <= epsilon[i], rand(price_numbers), greedy_policy[state,i])
	end
	return p			# p is updated by reference, but returning its value increses readibility
end


function get_next_state(memory::Array{Int32,1}, p::Array{Int8,1})
	# update memory (by reference) and get next state number
	if memory_length == 1
		memory[1] = get_one_memory_state(p)					# update memory
	elseif memory_length > 1 										
		memory[1:memory_length-1] = memory[2:memory_length]	# forget old memory
		memory[memory_length] = get_one_memory_state(p)		# update memory
	end
	return get_state_number(memory)
end
...
function init_greedy_policy(Q::Array{Float32,3}, alloc_max_idx::Array{Int8,1})
	...
	greedy_policy = Array{Int8, 2}(undef, n_states, n_agents)
...
function init_memory()
	...
	if memory_length == 0
		...
		return rand(one_memory_state, memory_length)
	end
end
